...
void CallGraph::addEdge(std::string caller, std::string callee, bool onThread) {
  std::cout << "Adding edge from " << caller << " to " << callee << ", with onthread=" << onThread
            << std::endl;
  ...
  // add (caller, callee, onThread) tuple to map. If it already exists, map is unaltered (|= is
  // idempotent)
...
void CallGraph::markNodes(std::vector<std::string> &startNodes, bool reverse = false) {
